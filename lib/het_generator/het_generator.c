...
void het_generator_init(){
	...
	logg(HET_GEN, "Loaded program at address: 0x%04X", pio_program_offset);


	float real_freq0 = het_generator_program_init(pio, 0, HET_GEN0_GPIO, HET_GEN0_FREQ);
	if((uint32_t)real_freq0 != (uint32_t)HET_GEN0_FREQ) logg(HET_GEN, "Generator[0] freq was changed -> Set: %dHz, Real: %.0fHz", HET_GEN0_FREQ, real_freq0);

	float real_freq1 = het_generator_program_init(pio, 1, HET_GEN1_GPIO, HET_GEN1_FREQ);
	if((uint32_t)real_freq1 != (uint32_t)HET_GEN1_FREQ) logg(HET_GEN, "Generator[1] freq was changed -> Set: %dHz, Real: %.0fHz", HET_GEN1_FREQ, real_freq1);

	float real_freq2 = het_generator_program_init(pio, 2, HET_GEN2_GPIO, HET_GEN2_FREQ);
	if((uint32_t)real_freq2 != (uint32_t)HET_GEN2_FREQ) logg(HET_GEN, "Generator[2] freq was changed -> Set: %dHz, Real: %.0fHz", HET_GEN2_FREQ, real_freq2);

	float real_freq3 = het_generator_program_init(pio, 3, HET_GEN3_GPIO, HET_GEN3_FREQ);
	if((uint32_t)real_freq3 != (uint32_t)HET_GEN3_FREQ) logg(HET_GEN, "Generator[3] freq was changed -> Set: %dHz, Real: %.0fHz", HET_GEN3_FREQ, real_freq3);
